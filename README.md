...
项目迁移使用首页的“导入旧电脑项目”：旧电脑导出项目包，新电脑先检查，再导入到 `项目名-from-old-computer` 的新目录。已有本机项目不会被合并或覆盖；默认保留 Git 历史，跳过依赖、缓存、构建产物、环境变量和密钥。

“迁移到另一台电脑”和“导入旧电脑项目”继续作为两个独立选择。导出不会删除旧电脑数据；确认新电脑内容正常后，可在“内容管理”中扫描并批量清理旧机副本。
