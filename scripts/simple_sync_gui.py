...
class SimpleApp(tk.Tk):
    ...
    def show_home(self):
        ...
        actions = (
            ...
            ("导入旧电脑项目", "保留本机项目，把旧电脑项目导入独立目录。", self.show_project_transfer),
    ...
    def show_content_manager(self):
        ...
        consistency_row.pack(fill="x", pady=(0, 8))
        ...
        self.manager_consistency_button = ttk.Button(
            consistency_row, text="一致性报告", command=self.show_manager_consistency, state="disabled"
        )
        self.manager_consistency_button.pack(side="right", padx=(7, 0))
        self.manager_clean_stale_button = ttk.Button(
            consistency_row, text="清理侧栏残留", command=self.clean_stale_sidebar_metadata, state="disabled"
        )
        self.manager_clean_stale_button.pack(side="right")
        ...
        self.manager_trash_projects_button = ttk.Button(
            project_controls, text="项目移入回收区", command=self.trash_managed_projects, state="disabled"
        )
        self.manager_trash_projects_button.pack(side="right", padx=7)
        ttk.Button(project_controls, text="恢复最近移除项目", command=self.restore_latest_project).pack(side="right")
        ...
        self.manager_action_buttons = [
            ...
            self.manager_trash_projects_button,
    ...
    def configure_manager_project_filter(self, previous_project_path=None):
        ...
        for path, items in grouped.items():
            name = items[0].get("project_name") or "未关联"
    ...
    def trash_managed_projects(self):
        selected = self.project_manager_tree.selection()
        paths = [Path(self.manager_projects[item_id]["path"]) for item_id in selected if self.manager_projects[item_id]["exists"]]
        if not paths:
            messagebox.showwarning(APP_NAME, "请选择仍然存在的项目目录。")
            return
        if not messagebox.askyesno(
            APP_NAME,
            f"将把 {len(paths)} 个项目目录移入软件的项目回收区。\n关联对话不会自动删除，可单独处理。\n\nCodex 必须完全关闭。是否继续？",
        ):
            return
        self._begin_manager_action()
        self.run(
            "正在把项目移入可恢复区...",
            lambda: content_manager.move_projects_to_trash(paths),
            lambda result: self.complete_manager_action(
                result, f"已移动 {result['moved']} 个项目。\n项目回收区：{result['trash_root']}"
            ),
        )
    ...
    def restore_latest_project(self):
        ...
        self.run(
            "正在恢复项目...",
            lambda: content_manager.restore_project(Path(latest["item_root"])),
            lambda result: self.complete_manager_action(result, f"项目已恢复：\n{result['restored_path']}"),
        )
    ...
    def show_project_transfer(self):
        self.clear()
        self.nav()
        self.title_block("导入旧电脑项目", "本机已有项目不会修改；旧电脑项目会载入一个全新的独立目录。")
        choice = ttk.Frame(self.content)
        choice.pack(fill="x", pady=(0, 18))
        ttk.Button(choice, text="我在旧电脑：导出项目", command=self.show_project_export).pack(side="left", ipady=8)
        ttk.Button(choice, text="我在新电脑：导入项目", command=self.show_project_import).pack(side="left", padx=12, ipady=8)
    ...
    def export_project(self):
        def operation():
            return project_import.create_project_bundle(
    ...
    def show_project_import(self):
        self.clear()
        self.nav()
        self.title_block("保留本机项目并导入", "先检查迁移包。实际导入仅创建新的项目目录，不会合并或覆盖已有项目。")
        ...
        detail_frame = ttk.LabelFrame(self.content, text="项目导入预览", padding=8)
    ...
    def check_project_import(self):
        ...
        def operation():
            ...
            prepared = project_import.prepare_project_import(bundle, Path(self.project_root.get()))
            return {
                "bundle": str(bundle),
    ...
    def show_project_import_preview(self, preview):
        ...
        metadata = preview["manifest"]["metadata"]
        ...
        lines = [
            ...
            "",
            "开始导入只会创建上述新目录。已有本机项目不会被扫描、合并或覆盖。",
            "导入前若该目标目录已被其他操作创建，导入会停止并要求重新检查。",
        ]
    ...
    def import_project(self):
        ...
        if not messagebox.askyesno(
            APP_NAME,
            "将创建新的项目目录，不会修改本机已有项目。\n\n导入前会再次确认目标目录不存在，并创建可恢复备份。继续？",
        ):
            return
        ...
        def operation():
            return project_import.restore_project_bundle(
                Path(preview["bundle"]),
    ...
    def complete_project_import(self, result):
        ...
        messagebox.showinfo(
            APP_NAME,
            f"项目已导入为独立目录。\n\n项目位置：{result['project_path']}\n备份位置：{result['backup_path']}\n\n请在 Codex 或 IDE 中打开该项目目录。",
        )
